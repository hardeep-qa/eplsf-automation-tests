...
Scenario Outline: SignUp with invalid credentials
	...
	|	      | test4@test.com | test	  | test	        | China (ii-CN)		  | Name required  		                |
	...
	| hardeep | test4@test.com | test	  |       	        | Morocco (ar-MA)	  | Passwords do not match 			    |
	| hardeep | test4@test.com | test	  | tset    	    | Russia (ba-RU)	  | Passwords do not match 			    |
...
Scenario: Verify SignUp with already registered account and error message link functionality
	...
	And User enter the "test4@test.com" Email Address on signup page
